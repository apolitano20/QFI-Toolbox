...
if(opMode)
    ...
    inputMat = mat2cell(QFIEntanglementToolbox.PermuteSystems(inputMat,permVec,dimVec), subSysVec, subSysVec); % permute the subsystems so that we just have to do the partial trace on the first (potentially larger) subsystem
...
% Otherwise, do a clever trick with mat2cell or reshaping, which is almost always faster.
    ...
    outputMat = QFIEntanglementToolbox.PermuteSystems(inputMat,permVec,dimVec); % permute the subsystems so that we just have to do the partial trace on the second (potentially larger) subsystem
