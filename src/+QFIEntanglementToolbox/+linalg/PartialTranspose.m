...
outputMat = QFIEntanglementToolbox.linalg.PermuteSystems(inputMat,permVec,dimArray); % permute the subsystems so that we just have to do the partial transpose on the first (potentially larger) subsystem
...
outputMat = QFIEntanglementToolbox.linalg.PermuteSystems(outputMat,permVec,dimArray,0,1);
end
